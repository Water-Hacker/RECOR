//! TODO-040 — operator-facing DLQ endpoints for the person service.
//!
//!   GET  /v1/internal/outbox-dlq               — list dead-lettered rows
//!   POST /v1/internal/outbox-dlq/{id}/replay   — atomically move a row
//!                                                 back to outbox for retry
//!
//! Mirrors `services/declaration/src/api/dlq.rs` and
//! `services/entity-service/src/api/dlq.rs`. The operator-side contract
//! — query params, response shape, auth gate, 503-on-empty-allowlist —
//! is identical so a single dashboard / on-call runbook covers every
//! service.
//!
...
//! admin surface (D14 fail-closed + D17 zero-trust).
...
use crate::infrastructure::outbox_admin::{DlqRow, OutboxAdminError, OutboxAdminStore};
...
#[instrument(skip_all, fields(principal = %principal.subject, limit = query.limit, offset = query.offset))]
...
fn backend_error(e: OutboxAdminError) -> (StatusCode, Json<serde_json::Value>) {
    tracing::error!(error = ?e, "outbox-admin backend failure");
    error_response(
...
mod tests {
    ...
    fn admin_endpoint_refuses_non_admin_principal() {
        ...
        let p = principal("spiffe://recor.cm/regular-caller");
        let err = enforce_admin(&set, &p).expect("expected refusal");
