...
//!     configured issuer's JWKS via `crate::api::oidc::OidcVerifier`.
//!     Real signature + iss + aud + exp + nbf checking; HMAC algorithms
//!     are refused outright.
//!   - Dev: an `X-Recor-Dev-Principal` header shortcut. Gated by
//!     `Config::is_dev()`; refused otherwise.
//!
//! D14 (fail-closed): bearer-token requests with no verifier configured
//! are rejected with 401, not silently allowed through. The config layer
//! refuses to start outside dev when `OIDC_ISSUER_URL` is empty, so a
//! production deployment cannot land in the "no verifier" state.
//!
