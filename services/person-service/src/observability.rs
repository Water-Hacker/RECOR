...
//!
//! Mirrors `services/declaration/src/observability.rs` to keep the
//! operational surface consistent across services. The PII-redacting
//! layer from `recor-logging` is installed first in the layer stack so
//! its side effects (overwriting span field values in the registry) are
//! visible to the downstream fmt + OTel layers.
...
pub fn init(cfg: &Config) -> Result<TracingGuard, ObservabilityError> {
    ...
    let tracer = provider.tracer("recor-person-service");
...
fn build_redaction(
    cfg: &Config,
) -> Result<recor_logging::RedactionConfig, ObservabilityError> {
