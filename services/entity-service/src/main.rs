...
use recor_entity_service::infrastructure::{
    warn_if_misconfigured, IdempotencyStore, OutboxAdminStore, OutboxRelay, OutboxRetention,
    PostgresEntityRepository, RelaySubscriber,
};
...
async fn main() -> Result<()> {
    ...
    // TODO-039 — DLQ admin store. Shared with the DLQ admin API state.
    ...
    // FIND-018 / R-LOOP-3: SPIFFE bootstrap. Same shape as
    // person-service / V-engine / declaration. Hard-fail under
    // AUTH_TRANSPORT=mtls if the Workload API is unavailable (D14).
    ...
    let spiffe_client = if cfg.mtls_enabled() {
        ...
        // TODO(R-LOOP-3-followup): when entity-service grows an
        // inbound internal endpoint, wire the peer-SPIFFE-ID gate
        // using `recor_spiffe::enforce_peer_id`. Test pattern lives
        // at `services/verification-engine/tests/peer_spiffe_id_gate.rs`.
        Some(client)
    } else {
        info!(
            transport = %cfg.auth_transport,
    ...
    // TODO-039 — outbox relay worker. Optional: enabled iff
    ...
    let relay_handle = if !cfg.outbox_relay_target_url.is_empty() {
        ...
        info!(
            ...
            "entity outbox relay enabled"
        );
        Some(tokio::spawn(async move {
            relay.run(cancel_relay).await;
        }))
    } else {
        info!("entity outbox relay disabled (OUTBOX_RELAY_TARGET_URL not set)");
        None
    };
