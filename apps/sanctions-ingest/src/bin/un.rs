//! TODO-014-UN — UN Consolidated sanctions ingest binary.
//!
//! Usage:
//!   recor-sanctions-ingest-un \
//!     --source-file <path-to-consolidated.xml> \
//!     [--force "<justification>"] \
//!     [--max-drop-ratio 0.25]
//!
...
fn parse_args() -> Result<Args> {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: recor-sanctions-ingest-un --source-file <path> [--force <justification>] [--max-drop-ratio 0.25]"
                );
                std::process::exit(0);
            }
    ...
    Ok(Args {
        source_file,
        source_url: std::env::var("UN_CONSOLIDATED_URL").ok(),
        force_justification,
        max_drop_ratio,
...
async fn main() -> ExitCode {
    ...
    match real_main().await {
        Ok(code) => code,
        Err(e) => {
            error!(error = ?e, "un consolidated ingest failed");
            ExitCode::from(2)
        }
    }
}
...
async fn real_main() -> Result<ExitCode> {
    ...
    let bytes: Vec<u8> = if let Some(path) = args.source_file.as_ref() {
        ...
        info!(%url, "fetching UN Consolidated feed");
        ...
        client
            ...
            .send()
            .await
            .context("UN Consolidated fetch")?
            .error_for_status()
            .context("UN Consolidated HTTP status")?
            .bytes()
            .await
            .context("UN Consolidated body read")?
            .to_vec()
    } else {
        anyhow::bail!("either --source-file or UN_CONSOLIDATED_URL env must be set");
    };
    let digest_hex = blake3::hash(&bytes).to_hex().to_string();
    info!(
        bytes_read = bytes.len(),
        digest = %&digest_hex[..16],
        "UN Consolidated bytes ready"
    );

    let entries = parse_un(&bytes).context("UN Consolidated parse")?;
    let proposed: u64 = entries.len() as u64;
    info!(proposed_row_count = proposed, "UN Consolidated parsed");
    ...
    let prior_row: (i64,) = sqlx::query_as(
        r#"SELECT COUNT(*)::bigint FROM sanctions_persons WHERE source = 'un_consolidated'"#,
    )
    .fetch_one(&pool)
    .await
    ...
    if blocked && !forced {
        let entry = IngestLogEntry {
            source: "un_consolidated".to_string(),
        ...
        warn!(
            ...
            "UN Consolidated ingest BLOCKED by sanity check; re-run with --force '<justification>' to override"
        );
        return Ok(ExitCode::from(6));
    }

    let applied = upsert_un_entries(&pool, &entries)
        .await
        .context("UN Consolidated upsert")?;
    info!(applied, "UN Consolidated upsert complete");
